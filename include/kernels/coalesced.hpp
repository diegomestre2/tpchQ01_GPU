...
void thread_local_tpchQ01_small_datatypes_coalesced(
	...
	record_count_t*                      __restrict__ record_count,
	...
	record_count_t                                    cardinality)
 {
	...
	record_count_t         thread_record_count         [num_potential_groups] = { 0 };

	record_count_t i = (blockIdx.x * blockDim.x + threadIdx.x);
	record_count_t stride = (blockDim.x * gridDim.x); //Grid-Stride
	for(; i < cardinality; i += stride) {
		if (shipdate[i] <= compressed_threshold_ship_date) {
			...
			auto line_status_          = get_bit_resolution_element<log_line_status_bits, record_count_t>(line_status, i);
			auto line_return_flag      = get_bit_resolution_element<log_return_flag_bits, record_count_t>(return_flag, i);
