...
#
# * TODO: We must switch this to running nvprof with kernel serialization and 1 stream only, and obtaining the total kernel time (and perhaps invocation count)
...
default_num_runs = 1 # this is sufficient; the profiler will take care of repetition
...
# We don't care about the co-processing, since that doesn't affect the kernel code at all, just the workload
options = [
#	"",
	"--apply-compression",
...
for opt in options:
	for vals in tuples_per_thread:
		for threads in threads_per_block:
			for p in placements:
					run_test(raw_results_file = raw_fn, mean_results_file = mean_fn, filename_for_plot=os.path.join(gp_basename, 'vals_%s-threads_%s-placement_%s.csv' % (str(vals), str(threads), p)), vals = vals, threads = threads, placement = p, options = opt)
